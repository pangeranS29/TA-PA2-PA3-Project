...
import { previewLaporanDewasa, exportLaporanDewasa } from "../services/laporan";
import {
	Download,
	ArrowLeft,
	Loader2,
...
export default function LaporanDewasaPreview() {
	...
	const fetchPreview = async () => {
		...
		try {
			...
			if (filterEnabled && startDate && endDate) {
				rawData = await previewLaporanDewasa(startDate, endDate);
			} else {
				rawData = await previewLaporanDewasa();
			}
			...
			if (normalized.length === 0 && filterEnabled) {
				...
				setError("Belum ada data laporan dewasa yang tersedia");
			}
		} catch (err) {
			console.error("Preview error:", err);
			const msg = err.response?.data?.message || err.message || "Gagal memuat preview data dewasa";
			setError(msg);
			setData([]);
		} finally {
			setLoading(false);
	...
	const handleExport = async () => {
		...
		try {
			let blob;
			if (filterEnabled && startDate && endDate) {
				blob = await exportLaporanDewasa(startDate, endDate);
			} else {
				blob = await exportLaporanDewasa();
			}
			const url = window.URL.createObjectURL(blob);
			const a = document.createElement("a");
			a.href = url;
			a.download = `laporan_dewasa_${filterEnabled ? `${startDate}_to_${endDate}` : "semua"}.xlsx`;
			a.click();
			...
			Swal.fire({
				...
				text: "Gagal mengekspor laporan dewasa: " + (err.response?.data?.message || err.message),
				...
				<div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 mb-6">
					<div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
							...
							<div className="flex flex-wrap items-center gap-3">
								...
								<div className="flex items-center gap-2">
									<input
										...
										className="border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-purple-300 bg-white"
									/>
								...
								<div className="flex gap-2">
									<button
										...
										className="bg-purple-600 text-white px-3 py-1.5 rounded-lg text-sm font-medium hover:bg-purple-700 transition shadow-sm"
									>
										Terapkan Filter
									</button>
									{filterEnabled && (
						...
						<button
							...
							className="bg-purple-600 hover:bg-purple-700 text-white px-5 py-2 rounded-lg flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition shadow-sm w-full lg:w-auto"
						>
					...
					{filterEnabled && startDate && endDate && (
						<div className="mt-3 text-xs text-purple-600 bg-purple-50 p-2 rounded-lg inline-flex items-center gap-1">
				...
				{!loading && error && (
					<div className="bg-red-50 border border-red-200 rounded-xl p-6 text-center">
						...
						<button
							...
							className="mt-4 inline-flex items-center gap-2 text-purple-600 text-sm hover:underline"
						>
