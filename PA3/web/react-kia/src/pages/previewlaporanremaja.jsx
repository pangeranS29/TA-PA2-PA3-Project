...
import { previewLaporanRemaja, exportLaporanRemaja } from "../services/laporan";
import {
	Download,
	ArrowLeft,
	Loader2,
...
export default function LaporanRemajaPreview() {
	...
	const fetchPreview = async () => {
		...
		try {
			...
			if (filterEnabled && startDate && endDate) {
				rawData = await previewLaporanRemaja(startDate, endDate);
			} else {
				rawData = await previewLaporanRemaja();
			}
			...
			if (normalized.length === 0 && filterEnabled) {
				...
				setError("Belum ada data laporan remaja yang tersedia");
			}
		} catch (err) {
			console.error("Preview error:", err);
			const msg = err.response?.data?.message || err.message || "Gagal memuat preview data remaja";
			setError(msg);
			setData([]);
		} finally {
			setLoading(false);
	...
	const handleExport = async () => {
		...
		try {
			let blob;
			if (filterEnabled && startDate && endDate) {
				blob = await exportLaporanRemaja(startDate, endDate);
			} else {
				blob = await exportLaporanRemaja();
			}
			const url = window.URL.createObjectURL(blob);
			const a = document.createElement("a");
			a.href = url;
			a.download = `laporan_remaja_${filterEnabled ? `${startDate}_to_${endDate}` : "semua"}.xlsx`;
			a.click();
			...
			Swal.fire({
				...
				text: "Gagal mengekspor laporan remaja: " + (err.response?.data?.message || err.message),
