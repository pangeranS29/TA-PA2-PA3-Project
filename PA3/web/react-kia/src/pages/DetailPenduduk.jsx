...
export default function DetailPenduduk() {
  ...
  const { data_diri, riwayat } = data;
  const latest = riwayat.length > 0 ? riwayat[0] : null;
  const tglLahir = data_diri.tanggal_lahir ? new Date(data_diri.tanggal_lahir).toLocaleDateString('id-ID') : '-';
            ...
            {!latest ? (
              ...
              <>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <div className="text-gray-500">TD (Tekanan Darah)</div>
                    <div className="font-medium">{formatValue(latest.tekanan_darah)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">TB (Tinggi Badan)</div>
                    <div className="font-medium">{latest.tinggi_badan ? `${latest.tinggi_badan} cm` : '-'}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">BB (Berat Badan)</div>
                    <div className="font-medium">{latest.berat_badan ? `${latest.berat_badan} kg` : '-'}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">CP (Catatan Pemeriksaan)</div>
                    <div className="font-medium">{formatValue(latest.catatan_khusus)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">HB (Hemoglobin)</div>
                    <div className="font-medium">{formatValue(latest.hemoglobin)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">GD (Gula Darah)</div>
                    <div className="font-medium">{latest.gula_darah ? `${latest.gula_darah} mg/dL` : '-'}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">cUA (Asam Urat)</div>
                    <div className="font-medium">{formatValue(latest.asam_urat)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">AU (Asam Urat)</div>
                    <div className="font-medium">{formatValue(latest.asam_urat)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">COL (Kolesterol)</div>
                    <div className="font-medium">{latest.kolesterol ? `${latest.kolesterol} mg/dL` : '-'}</div>
                  </div>
                </div>

                {/* Indikator "Data bulan sebelum" jika ada riwayat lain */}
                {riwayat.length > 1 && (
                  <div className="mt-4 pt-3 text-xs text-gray-400 italic border-t">
                    Data bulan sebelum: {new Date(latest.tanggal_pemeriksaan).toLocaleDateString('id-ID')}
                  </div>
                )}
              </>
            )}
            ...
            <div className="space-y-3">
              {riwayat.map((item) => (
                <div key={item.id} className="border rounded-lg overflow-hidden">
                  ...
                  >
                    <div>
                      <span className="font-medium">{new Date(item.tanggal_pemeriksaan).toLocaleDateString('id-ID')}</span>
                      <span className="ml-2 text-sm text-gray-500">({item.kategori})</span>
                      <span className={`ml-2 inline-block px-2 py-0.5 rounded-full text-xs ${
                        item.kategori_risiko === 'Tinggi' ? 'bg-red-100 text-red-700' :
                        item.kategori_risiko === 'Sedang' ? 'bg-yellow-100 text-yellow-700' :
                        'bg-green-100 text-green-700'
                      }`}>Risiko: {item.kategori_risiko}</span>
                    </div>
                  ...
                  {expandedId === item.id && (
                    <div className="px-4 py-3 bg-white border-t text-sm space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        {item.imt !== null && <div><span className="text-gray-600">IMT:</span> {item.imt.toFixed(1)}</div>}
