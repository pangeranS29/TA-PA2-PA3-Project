...
function mapItemToForm(item) {
  return {
    judul: item.judul || '',
    slug: item.slug || '',
    kategori: item.kategori || 'PHBS',
    phase: item.phase || 'semua',
...
export default function AdminInformasi() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true)
      const res = await adminApi.get('/admin/content?feature=informasi_umum&page=1&limit=200')
      setItems(Array.isArray(res?.data?.data) ? res.data.data : [])
    } catch {
      toast.error('Gagal memuat data informasi umum')
      setItems([])
    } finally {
      setLoading(false)
    }
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await adminApi.delete(`/admin/content/${item.id}?feature=informasi_umum`)
  ...
  const submitForm = async (publishValue) => {
    ...
    try {
      ...
      if (mode === 'edit' && editingItem?.id) {
        await adminApi.put(`/admin/content/${editingItem.id}?feature=informasi_umum`, payload)
        toast.success('Perubahan disimpan')
      } else {
        await adminApi.post('/admin/content?feature=informasi_umum', payload)
      ...
      <AdminLayout>
        <div className="parenting-admin-page">
          <section className="parenting-admin-head">
            <div>
              <h1>Manajemen Informasi Umum</h1>
              <p>Kelola artikel PHBS, kesehatan gigi, keamanan, dan lainnya.</p>
            </div>
                  ...
                  {loading ? (
                    ...
                    paged.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td>
                          <span className="parenting-admin-pill age">{item.phase === 'semua' ? 'Semua' : item.phase || '-'}</span>
                        </td>
                        <td>
    ...
    <AdminLayout>
      <div className="parenting-admin-page">
        <section className="parenting-admin-form-topbar">
          <div>
            <h1>{mode === 'edit' ? 'Edit Artikel Informasi Umum' : 'Tambah Artikel Informasi Umum'}</h1>
          </div>
          <div>
        ...
        <section className="parenting-admin-form-grid">
          <div className="parenting-admin-form-main">
            <article className="parenting-admin-card">
              <h3><Info size={16} /> Informasi Utama</h3>
              ...
              <input
                ...
                placeholder="Contoh: Cara Menyehatkan Gigi Anak"
              />

              <label>Slug URL</label>
              <input value={form.slug} onChange={(e) => updateField('slug', e.target.value)} placeholder="cara-menyehatkan-gigi-anak" />
          ...
          <aside className="parenting-admin-form-side">
            ...
            <article className="parenting-admin-card">
              <h3><Shield size={16} /> Fase Target</h3>
              <select value={form.phase} onChange={(e) => updateField('phase', e.target.value)}>
                {PHASE_OPTIONS.map((item) => (
                  <option key={item} value={item}>
                    {item === 'kehamilton' ? 'Kehamilton' : item === 'bayi' ? 'Bayi' : item === 'balita' ? 'Balita' : 'Semua Usia'}
                  </option>
                ))}
              </select>
            </article>
