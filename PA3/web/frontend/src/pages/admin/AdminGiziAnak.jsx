...
function mapItemToForm(item) {
  return {
    judul: item.judul || '',
    slug: item.slug || '',
    kategori: item.kategori || 'Nutrisi',
    phase: item.phase || 'mpasi_6_24',
...
export default function AdminGiziAnak() {
  ...
  const fetchItems = async () => {
    try {
      setLoading(true)
      const res = await adminApi.get('/admin/content?feature=gizi_anak&page=1&limit=200')
      setItems(Array.isArray(res?.data?.data) ? res.data.data : [])
    } catch {
      toast.error('Gagal memuat data gizi anak')
      setItems([])
    } finally {
      setLoading(false)
    }
  ...
  const filtered = useMemo(() => {
    ...
    if (ageFilter !== 'all') {
      data = data.filter((item) => item.phase === ageFilter)
    }
    ...
    return data
  }, [items, search, ageFilter, statusFilter])
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await adminApi.delete(`/admin/content/${item.id}?feature=gizi_anak`)
  ...
  const submitForm = async (publishValue) => {
    ...
    try {
      ...
      if (mode === 'edit' && editingItem?.id) {
        await adminApi.put(`/admin/content/${editingItem.id}?feature=gizi_anak`, payload)
        toast.success('Perubahan disimpan')
      } else {
        await adminApi.post('/admin/content?feature=gizi_anak', payload)
      ...
      <AdminLayout>
        <div className="parenting-admin-page">
          <section className="parenting-admin-head">
            <div>
              <h1>Manajemen Gizi Anak</h1>
              <p>Kelola artikel gizi untuk bayi dan anak.</p>
            </div>
            ...
            <div className="parenting-admin-table-toolbar">
              ...
              >
                ...
                {AGE_OPTIONS.map((item) => (
                  <option key={item} value={item}>{item === 'bayi_0_6' ? 'Bayi 0-6 Bulan' : item === 'mpasi_6_24' ? 'MPASI 6-24 Bulan' : 'Balita 2-5 Tahun'}</option>
                ))}
              </select>
                ...
                <tbody>
                  {loading ? (
                    ...
                    <tr><td colSpan={6} className="parenting-admin-empty">
                      <ChefHat size={32} />
                      <p>Belum ada artikel gizi anak.</p>
                    ...
                    paged.map((item) => (
                      <tr key={item.id}>
                        ...
                        <td>
                          <span className="parenting-admin-pill age">{item.phase === 'bayi_0_6' ? '0-6 Bulan' : item.phase === 'mpasi_6_24' ? '6-24 Bulan' : item.phase === 'balita_2_5' ? '2-5 Tahun' : item.phase || '-'}</span>
                        </td>
                        <td>
    ...
    <AdminLayout>
      <div className="parenting-admin-page">
        <section className="parenting-admin-form-topbar">
          <div>
            <h1>{mode === 'edit' ? 'Edit Artikel Gizi Anak' : 'Tambah Artikel Gizi Anak'}</h1>
          </div>
          <div>
          ...
          <div className="parenting-admin-form-main">
            <article className="parenting-admin-card">
              ...
              <input
                ...
                placeholder="Contoh: Makanan Sumber Protein untuk Bayi"
              />

              <label>Slug URL</label>
              <input value={form.slug} onChange={(e) => updateField('slug', e.target.value)} placeholder="makanan-sumber-protein-bayi" />
              ...
              <select value={form.phase} onChange={(e) => updateField('phase', e.target.value)}>
                {AGE_OPTIONS.map((item) => (
                  <option key={item} value={item}>
                    {item === 'bayi_0_6' ? 'Bayi 0-6 Bulan' : item === 'mpasi_6_24' ? 'MPASI 6-24 Bulan' : 'Balita 2-5 Tahun'}
                  </option>
                ))}
              </select>
            </article>
