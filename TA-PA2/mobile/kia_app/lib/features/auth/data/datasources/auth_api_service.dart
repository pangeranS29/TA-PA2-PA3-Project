...
class AuthApiService {
  ...
  Future<void> login({
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw Exception(
        'Login gagal (${response.statusCode}). Periksa email/no hp dan password.',
      );
    }
    ...
    final token = (data['access_token'] ?? '').toString();
    final role = (data['role'] ?? '').toString();
    final name = (data['name'] ?? 'User KIA').toString();

    if (token.isEmpty) {
      throw Exception('Access token tidak ditemukan di response login.');
    }
