...
//                     _buildInfoRow(
//                       'Status Kehamilan',
//                       profil.statusKehamilan.isEmpty
//                           ? '-'
...
class _ProfilScreenState extends State<ProfilScreen>
  ...
  Widget _buildContent(ProfilIbuModel profil) {
    return SingleChildScrollView(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // ── Header biru (konsisten dengan Edukasi & Absensi) ──
          Container(
            width: double.infinity,
            color: AppColors.primary,
            child: SafeArea(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            profil.namaLengkap.isEmpty ? 'Profil' : profil.namaLengkap,
                            style: const TextStyle(
                              color: Colors.white,
                              fontSize: 17,
  ...
  // Chip putih transparan untuk header biru
  ...
  // Chip berwarna untuk dalam card riwayat
  Widget _statusChip(String status) {
    Color bg;
    Color fg;
    switch (status.toLowerCase()) {
          ...
          Wrap(
            ...
            children: [
              _miniInfo('G${item.gravida} P${item.paritas} A${item.abortus}',
                  Icons.pregnant_woman),
              if (item.hpht.isNotEmpty)
                _miniInfo('HPHT: ${_formatDate(item.hpht)}', Icons.calendar_today),
              if (item.taksiranPersalinan.isNotEmpty)
                _miniInfo('HPL: ${_formatDate(item.taksiranPersalinan)}', Icons.event),
              if (item.bbAwal > 0)
                _miniInfo('BB: ${item.bbAwal.toStringAsFixed(1)} kg',
                    Icons.monitor_weight_outlined),
              if (item.tb > 0)
                _miniInfo('TB: ${item.tb.toStringAsFixed(1)} cm', Icons.height),
            ],
          ),
        ],
      ),
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 3),
                            Text(
                              '${anak.jenisKelamin}  •  ${anak.usiaTeks}',
                              style: TextStyle(
                                  fontSize: 12,
                                  color: AppColors.textSecondary),
                            ),
