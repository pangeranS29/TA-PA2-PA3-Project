...
import 'pemantauan_ibu_nifas_kader_api_service.dart';
import 'pemantauan_ibu_nifas_kader_model.dart';

class VerifikasiPemantauanIbuNifasScreen extends StatefulWidget {
  const VerifikasiPemantauanIbuNifasScreen({super.key});

  @override
  State<VerifikasiPemantauanIbuNifasScreen> createState() =>
      _VerifikasiPemantauanIbuNifasScreenState();
}

class _VerifikasiPemantauanIbuNifasScreenState
    extends State<VerifikasiPemantauanIbuNifasScreen> {
  final _apiService = PemantauanIbuNifasKaderApiService();

  List<PemantauanIbuNifasKaderModel> _list = [];
  bool _isLoading = true;
  String _errorMessage = '';
  bool? _filterVerified;

  // Warna tema nifas (ungu/lavender)
  static const Color _primaryColor = Color(0xFF7B52AB);
  ...
  List<PemantauanIbuNifasKaderModel> get _filtered {
    if (_filterVerified == null) return _list;
    return _list
        .where((item) => item.sudahDitinjau == _filterVerified)
        .toList();
  }
  ...
  Future<void> _verify(PemantauanIbuNifasKaderModel item) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Berhasil meninjau data pemantauan ibu nifas'),
  ...
  Widget _filterChip(
    ...
    final activeColor = color ?? _primaryColor;
  ...
  Widget _buildCard(PemantauanIbuNifasKaderModel item) {
    ...
    final String badgeLabel = isVerified ? 'Sudah Ditinjau' : 'Belum Ditinjau';

    final bool adaKeluhan = item.jumlahKeluhan > 0;
    final Color keluhanColor =
        adaKeluhan ? Colors.red.shade400 : Colors.grey.shade400;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Status checklist positif (nifas A)
          _buildStatusRow(
            icon: Icons.check_circle_outline,
            label: 'Pemeriksaan nifas',
            aktif: item.pemeriksaanNifas,
          ),
          const SizedBox(height: 4),
          _buildStatusRow(
            icon: Icons.vaccines_outlined,
            label: 'Konsumsi Vitamin A',
            aktif: item.konsumsiVitaminA,
          ),
          const SizedBox(height: 4),
          _buildStatusRow(
            icon: Icons.restaurant_outlined,
            label: 'Pemenuhan gizi',
            aktif: item.pemenuhanGizi,
          ),

          // Ringkasan keluhan tanda bahaya
          const SizedBox(height: 10),
          Row(
            children: [
              Icon(
              ...
              Text(
                adaKeluhan
                    ? '${item.jumlahKeluhan} tanda bahaya tercatat'
          ...
          if (adaKeluhan) ...[
            ...
            Wrap(
              spacing: 6,
              runSpacing: 6,
              children: item.daftarKeluhan
                  .map((k) => _keluhanChip(k))
                  .toList(),
            ),
          ],

          // Keluhan tambahan teks
          if (item.keluhan.isNotEmpty) ...[
            const SizedBox(height: 8),
            Container(
    ...
    return Row(
      children: [
        Icon(
          ...
          size: 14,
          color: aktif ? Colors.green.shade600 : Colors.grey.shade400,
        ),
        const SizedBox(width: 6),
        Text(
          label,
          style: TextStyle(
            fontSize: 12,
            color: aktif ? Colors.green.shade700 : Colors.grey.shade500,
          ),
        ),
      ],
    );
