...
class _ChecklistPemantauanIbuNifasScreenState
  ...
  Future<void> submitChecklist() async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('${ApiConstants.baseUrl}${ApiConstants.checklistPemantauanIbuNifas}'),
        headers: {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        await _showSuccessDialog(
          data['message']?.toString() ?? 'Checklist pemantauan ibu nifas berhasil disimpan',
        );
        if (!mounted) return;
  ...
  Future<void> _showSuccessDialog(String message) {
    return showDialog<void>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text('Checklist hari nifas ke-${selectedHariNifas ?? '-'} berhasil disimpan.'),
        actions: [
          TextButton(
  ...
  int get _dangerCount => [
        demamLebih38, sakitKepala, pandanganKabur, nyeriUluHati,
        masalahKesehatanJiwa, jantungBerdebar, cairanJalanLahir,
        napasPendek, payudaraBermasalah, gangguanBak, kelaminBermasalah,
        darahNifasBerbau, pendarahanBerat, keputihan,
      ].where((v) => v).length;
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          _daySelector(),
          selectedHariNifas == null ? _emptyChecklistHint() : _checklistForm(),
        ]),
      ),
    );
  }
  ...
  Widget _headerCard() {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const Text('Checklist Harian Masa Nifas', style: TextStyle(color: Colors.white, fontSize: 21, fontWeight: FontWeight.bold)),
        ...
        DropdownButtonFormField<int>(
          ...
          items: List.generate(42, (index) {
            ...
            return DropdownMenuItem<int>(
              ...
              enabled: !isFilled,
              child: Text(
                isFilled ? 'Hari nifas ke-$day - Sudah diisi' : 'Hari nifas ke-$day',
                overflow: TextOverflow.ellipsis,
                style: TextStyle(color: isFilled ? Colors.grey : Colors.black87),
              ),
            );
          }),
          onChanged: isLoading
              ...
              : (value) {
                  ...
                  if (widget.filledDays.contains(value)) {
                    _showError('Hari nifas ke-$value sudah diisi. Checklist hanya bisa diisi sekali sehari.');
                    return;
                  }
                  setState(() {
  ...
  Widget _checkItem({required String title, required bool value, required ValueChanged<bool?> onChanged}) {
    return Container(
      margin: const EdgeInsets.only(bottom: 10),
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: value ? Colors.blue.shade200 : Colors.grey.shade200),
  ...
  Widget _emptyChecklistHint() {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(22), border: Border.all(color: Colors.blue.shade50)),
  ...
  Widget _checklistForm() {
    return Column(children: [
      _dangerNotice(),
      _sectionTitle('Nifas A', 'Perawatan dasar dan gejala awal yang perlu dipantau.'),
      _checkItem(title: 'Pemeriksaan nifas', value: pemeriksaanNifas, onChanged: (v) => setState(() => pemeriksaanNifas = v ?? false)),
      _checkItem(title: 'Konsumsi vitamin A', value: konsumsiVitaminA, onChanged: (v) => setState(() => konsumsiVitaminA = v ?? false)),
      _checkItem(title: 'Pemenuhan gizi sesuai kebutuhan', value: pemenuhanGizi, onChanged: (v) => setState(() => pemenuhanGizi = v ?? false)),
      _checkItem(title: 'Demam > 38°C', value: demamLebih38, onChanged: (v) => setState(() => demamLebih38 = v ?? false)),
      _checkItem(title: 'Sakit kepala', value: sakitKepala, onChanged: (v) => setState(() => sakitKepala = v ?? false)),
      _checkItem(title: 'Pandangan kabur', value: pandanganKabur, onChanged: (v) => setState(() => pandanganKabur = v ?? false)),
      _checkItem(title: 'Nyeri ulu hati', value: nyeriUluHati, onChanged: (v) => setState(() => nyeriUluHati = v ?? false)),
      _checkItem(title: 'Masalah kesehatan jiwa', value: masalahKesehatanJiwa, onChanged: (v) => setState(() => masalahKesehatanJiwa = v ?? false)),
      _sectionTitle('Nifas B', 'Keluhan lanjutan yang perlu diperhatikan selama masa nifas.'),
      _checkItem(title: 'Jantung berdebar', value: jantungBerdebar, onChanged: (v) => setState(() => jantungBerdebar = v ?? false)),
      _checkItem(title: 'Keluar cairan dari jalan lahir', value: cairanJalanLahir, onChanged: (v) => setState(() => cairanJalanLahir = v ?? false)),
      _checkItem(title: 'Napas pendek / terengah-engah', value: napasPendek, onChanged: (v) => setState(() => napasPendek = v ?? false)),
      _checkItem(title: 'Payudara bengkak / bermasalah', value: payudaraBermasalah, onChanged: (v) => setState(() => payudaraBermasalah = v ?? false)),
      _checkItem(title: 'Gangguan BAK', value: gangguanBak, onChanged: (v) => setState(() => gangguanBak = v ?? false)),
      _checkItem(title: 'Area kelamin bengkak / nyeri / luka', value: kelaminBermasalah, onChanged: (v) => setState(() => kelaminBermasalah = v ?? false)),
      _checkItem(title: 'Darah nifas berbau', value: darahNifasBerbau, onChanged: (v) => setState(() => darahNifasBerbau = v ?? false)),
      _checkItem(title: 'Pendarahan berat', value: pendarahanBerat, onChanged: (v) => setState(() => pendarahanBerat = v ?? false)),
      _checkItem(title: 'Keputihan', value: keputihan, onChanged: (v) => setState(() => keputihan = v ?? false)),
