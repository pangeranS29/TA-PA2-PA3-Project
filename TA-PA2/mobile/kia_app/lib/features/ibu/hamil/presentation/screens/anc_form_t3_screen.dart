...
import 'package:ta_pa2_pa3_project/features/ibu/hamil/presentation/controllers/anc_t3_controller.dart';

class AncFormT3Screen extends StatefulWidget {
  const AncFormT3Screen({super.key});

  @override
  State<AncFormT3Screen> createState() => _AncFormT3ScreenState();
}
...
class _AncFormT3ScreenState extends State<AncFormT3Screen> {
          ...
          child: Form(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(children: [
                ...
                _buildSectionCard(title: 'Identitas Kunjungan', icon: Icons.calendar_month, color: t3Color, children: [
                  Row(children: [
                ...
                _buildSectionCard(title: 'Pemeriksaan Fisik', icon: Icons.monitor_weight_outlined, color: t3Color, children: [
                  Row(children: [
                ...
                // 3. LABORATORIUM
                _buildSectionCard(title: 'Laboratorium', icon: Icons.biotech_outlined, color: t3Color, children: [
                  Row(children: [
                  ...
                  EnumSelector(label: 'Protein Urin', options: const ['Negatif', 'Trace', '+1', '+2', '+3'], selectedValue: _controller.proteinUrin, activeColor: Colors.green, onChanged: (val) => setState(() => _controller.proteinUrin = val)),
                  EnumSelector(label: 'Urin Reduksi', options: const ['Negatif', '+1', '+2'], selectedValue: _controller.urinReduksi, activeColor: Colors.green, onChanged: (val) => setState(() => _controller.urinReduksi = val)),
                ]),

                // 4. USG TRIMESTER III
                _buildSectionCard(title: 'USG Trimester III', icon: Icons.analytics_outlined, color: t3Color, children: [
                  _buildInfoBanner('USG T3 menilai kesiapan persalinan: posisi janin, plasenta, ketuban, dan berat janin final.'),
                  const SizedBox(height: 16),
                  EnumSelector(label: 'Keadaan Bayi *', options: const ['Hidup', 'Meninggal'], selectedValue: _controller.keadaanBayi, activeColor: t3Color, onChanged: (val) => setState(() => _controller.keadaanBayi = val)),
                  EnumSelector(label: 'Lokasi Plasenta *', options: const ['Fundus', 'Corpus', 'Letak Rendah', 'Previa'], selectedValue: _controller.lokasiPlasenta, activeColor: t3Color, onChanged: (val) => setState(() => _controller.lokasiPlasenta = val)),
                  Row(children: [
                    ...
                    Expanded(child: Container(
                      ...
                      child: const Column(children: [
                        Text('Hasil', style: TextStyle(fontSize: 9, color: Colors.teal)),
                        Text('Cukup', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.teal)),
                      ]),
                    )),
                  ]),
                  const Divider(height: 32),
                  ...
                  _buildEfwBoxT3(),
                  const SizedBox(height: 16),
                  EnumSelector(label: 'Rencana Proses Melahirkan *', options: const ['Normal', 'Pervaginam Berbantu', 'Sectio Caesaria'], selectedValue: _controller.rencanaMelahirkan, activeColor: t3Color, onChanged: (val) => setState(() => _controller.rencanaMelahirkan = val)),
                ]),

                // 5. PREEKLAMPSIA UPDATE T3
                _buildSectionCard(title: 'Preeklampsia — Perbarui T3', icon: Icons.warning_amber_rounded, color: Colors.orange.shade800, children: [
                ...
                _buildSectionCard(title: 'Tata Laksana & Kesimpulan', icon: Icons.assignment_turned_in, color: t3Color, children: [
  ...
  Widget _buildDateField(String label, TextEditingController controller) {
    return TextFormField(
      controller: controller, readOnly: true, style: const TextStyle(fontSize: 14),
      decoration: InputDecoration(labelText: label,
        suffixIcon: IconButton(icon: const Icon(Icons.calendar_today, color: TrimesterTheme.t3Primary, size: 18), onPressed: () => _selectDate(context, controller)),
        border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
      ),
    );
  }

  Widget _buildFetalSection({required String label, required Color color, required String letakJanin, required TextEditingController djjCtrl, required String hasilDjj, required TextEditingController ketCtrl, required Function(String) onLetakChanged, required Function(String) onHasilChanged}) {
    return Container(padding: const EdgeInsets.all(12), margin: const EdgeInsets.only(top: 8), decoration: BoxDecoration(color: color.withOpacity(0.05), borderRadius: BorderRadius.circular(12), border: Border.all(color: color.withOpacity(0.2))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _statItem(String label, String value, String sub) {
    return Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.teal.shade100)), child: Column(children: [
  ...
  Widget _buildBiometriRow(String label, TextEditingController ctrl, String subLabel, String subVal) {
    return Padding(padding: const EdgeInsets.only(bottom: 12), child: Row(children: [
      ...
      const SizedBox(width: 12),
      Expanded(child: Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.teal.shade50, borderRadius: BorderRadius.circular(8)), child: Column(children: [
  ...
  Widget _buildEfwBoxT3() {
    return Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.teal.shade50, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.teal.shade100)), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
  ...
  Widget _buildReviewBox(String text) {
    return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.orange.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.orange.shade200)), child: Row(children: [
  ...
  Widget _buildScoreCircleT3() {
    return Center(child: Container(padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12), decoration: BoxDecoration(color: Colors.orange.shade900, borderRadius: BorderRadius.circular(30)), child: Row(mainAxisSize: MainAxisSize.min, children: [
  ...
  Widget _buildInfoBanner(String text) {
    return Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.teal.shade50, borderRadius: BorderRadius.circular(10)), child: Row(children: [
