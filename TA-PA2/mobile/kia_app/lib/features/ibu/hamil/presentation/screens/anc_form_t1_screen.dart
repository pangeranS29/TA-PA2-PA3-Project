...
import 'package:ta_pa2_pa3_project/features/ibu/hamil/presentation/controllers/anc_t1_controller.dart';

class AncFormT1Screen extends StatefulWidget {
  const AncFormT1Screen({super.key});

  @override
  State<AncFormT1Screen> createState() => _AncFormT1ScreenState();
}
...
class _AncFormT1ScreenState extends State<AncFormT1Screen> {
      ...
      builder: (context, child) => Theme(
        data: Theme.of(context).copyWith(
          colorScheme: const ColorScheme.light(
              primary: TrimesterTheme.t1Primary),
        ),
        child: child!,
      ),
    );
    ...
    if (_formKey.currentState!.validate()) {
      showDialog(
        ...
        builder: (context) => ConfirmationDialog(
          ...
          content:
              'Pastikan data sudah benar. Setelah disimpan, progres akan tercatat dan akses Trimester II akan terbuka.',
          primaryColor: color,
          onConfirm: () {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                  content: Text('Data Trimester I Berhasil Disimpan!')),
            );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Trimester I — ANC K1',
            style: TextStyle(
                  ...
                  children: [
                    ...
                    _buildSectionCard(
                      ...
                      children: [
                        ...
                        CustomInputField(label: 'DJJ', controller: _controller.djj, suffix: 'x/mnt'),
                      ],
                    ),
                    ...
                    _buildSectionCard(
                      ...
                      children: [
                        ...
                        EnumSelector(
                          ...
                          activeColor: t1Color,
                          onChanged: (val) => setState(() => _controller.jumlahGS = val),
                        ),
                        Row(children: [
                          Expanded(child: CustomInputField(label: 'Diameter GS *', controller: _controller.diameterGS, suffix: 'cm')),
                          const SizedBox(width: 12),
                          Expanded(child: CustomInputField(label: 'CRL *', controller: _controller.crl, suffix: 'cm')),
                        ]),
                        _buildPurpleAgeBox(),
                        const SizedBox(height: 16),
                        Row(children: [
                          Expanded(child: EnumSelector(
                            label: 'Pulsasi Jantung *',
                            options: const ['Tampak', 'Tidak'],
                            selectedValue: _controller.pulsasiJantung,
                            activeColor: Colors.green,
                            onChanged: (val) => setState(() => _controller.pulsasiJantung = val),
                          )),
                          ...
                          Expanded(child: EnumSelector(
                            ...
                            onChanged: (val) => setState(() => _controller.kecurigaanAbnormal = val),
                          )),
                        ]),
                      ],
                    ),
