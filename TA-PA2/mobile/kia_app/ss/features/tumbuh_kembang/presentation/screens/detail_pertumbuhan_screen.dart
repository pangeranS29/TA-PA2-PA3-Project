...
class DetailPertumbuhanScreen extends StatefulWidget {
  ...
  @override
  State<DetailPertumbuhanScreen> createState() => _DetailPertumbuhanScreenState();
}
...
class _DetailPertumbuhanScreenState extends State<DetailPertumbuhanScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final riwayat = await _repository.getRiwayatPertumbuhanByAnakId(widget.anak.id);
      ...
      final masterBBU = await _repository.getMasterStandar(parameter: 'bb_u', jenisKelamin: widget.anak.jenisKelamin);
      final masterTBU = await _repository.getMasterStandar(parameter: 'tb_u', jenisKelamin: widget.anak.jenisKelamin);
      final masterBBTB = await _repository.getMasterStandar(parameter: 'bb_tb', jenisKelamin: widget.anak.jenisKelamin);
      final masterIMTU = await _repository.getMasterStandar(parameter: 'imt_u', jenisKelamin: widget.anak.jenisKelamin);
      final masterLKU = await _repository.getMasterStandar(parameter: 'lk_u', jenisKelamin: widget.anak.jenisKelamin);
  ...
  List<MasterStandarModel> _getMasterForTab() {
    switch (_selectedTab) {
      case 'TB/U': return _masterStandarTBU;
      case 'BB/TB': return _masterStandarBBTB;
      case 'IMT/U': return _masterStandarIMTU;
      case 'LK/U': return _masterStandarLKU;
      case 'BB/U':
      default: return _masterStandarBBU;
    }
  }
  ...
  String _getStatusForTab(PertumbuhanModel data) {
    switch (_selectedTab) {
      case 'TB/U': return data.statusTBU;
      case 'BB/TB': return data.statusBBTB;
      case 'IMT/U': return data.statusIMTU;
      case 'LK/U': return data.statusLKU;
      case 'BB/U':
      default: return data.statusBBU;
    }
  }

  String _getXAxisLabel() => _selectedTab == 'BB/TB' ? 'Tinggi Badan (cm)' : 'Usia (bulan)';

  String _getYAxisLabelFull() {
    switch (_selectedTab) {
      case 'TB/U': return 'Tinggi Badan (cm)';
      case 'IMT/U': return 'Indeks Massa Tubuh (kg/m²)';
      ...
      child: Center(
        child: Column(
          children: [
            ...
            const SizedBox(height: 16),
            Text('Belum ada data pengukuran', style: TextStyle(fontSize: 14, color: Colors.grey.shade600)),
          ],
        ),
      ),
    );
  ...
  Widget _buildMeasurementInfoCard(PertumbuhanModel data) {
    ...
    switch (_selectedTab) {
      case 'TB/U': lastValue = data.tinggiBadan.toStringAsFixed(1); lastValueUnit = 'cm'; break;
      case 'IMT/U': lastValue = data.imt.toStringAsFixed(1); lastValueUnit = 'kg/m²'; break;
      case 'LK/U': lastValue = data.lingkarKepala.toStringAsFixed(1); lastValueUnit = 'cm'; break;
      case 'BB/U':
      case 'BB/TB':
      default: lastValue = data.beratBadan.toStringAsFixed(1); lastValueUnit = 'kg'; break;
    }
      ...
      child: Row(
        ...
        children: [
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('Pengukuran Terakhir', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Colors.grey.shade600)),
              const SizedBox(height: 8),
              RichText(
                text: TextSpan(
                  children: [
                    TextSpan(text: lastValue, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Color(0xFF2563EB))),
                    TextSpan(text: ' $lastValueUnit', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.grey.shade600)),
                  ],
                ),
              ),
            ],
          ...
          Column(
            ...
            children: [
              Text('Tanggal Ukur', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: Colors.grey.shade600)),
              const SizedBox(height: 8),
              Text(data.tglUkur, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.black87)),
            ],
          ),
        ],
      ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: isActive ? const Color(0xFF2563EB) : Colors.grey.shade300),
  ...
  Widget _buildRiwayatPengukuranCard() {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Riwayat pengukuran (4 Terbaru)', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
          const SizedBox(height: 16),
          ...reversedData.take(4).toList().asMap().entries.map((entry) {
            final index = entry.key;
            final data = entry.value;
            final isLatest = index == 0; // Karena sudah di reversed, 0 adalah terbaru
            return Column(
              children: [
                _buildRiwayatItem(data, isLatest),
                if (index != (reversedData.length > 4 ? 3 : reversedData.length - 1))
      ...
      child: Row(
        ...
        children: [
          Padding(
            ...
            child: Container(width: 12, height: 12, decoration: BoxDecoration(shape: BoxShape.circle, color: dotColor)),
          ),
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('${data.tglUkur} · ${data.usiaUkurBulan} bulan', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.black87)),
          ...
          Container(
            ...
            decoration: BoxDecoration(color: statusColors['bg'], borderRadius: BorderRadius.circular(12)),
            child: Text(statusText, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: statusColors['text'])),
          ),
        ],
      ),
    );
