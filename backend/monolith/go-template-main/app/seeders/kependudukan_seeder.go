	...
	for _, ibuNik := range ibuNikList {
		...
		if err := s.db.Where("nik = ?", ibuNik).First(&kepIbu).Error; err != nil {
			if err == gorm.ErrRecordNotFound {
				log.Printf("Kependudukan ibu with NIK %d not found, skipping Ibu seed\n", ibuNik)
				continue
			}
			return err
		}
...
func (s *KependudukanSeeder) seedAnak() error {
	...
	for nik, detailAnak := range dataAnakMap {
		...
		if detailAnak.NoKK > 0 {
			if err := s.db.Where("no_kk = ?", fmt.Sprintf("%d", detailAnak.NoKK)).First(&kartuKeluarga).Error; err != nil {
				if err != gorm.ErrRecordNotFound {
					return err
				}
			}
		...
		var kependudukanID *int32

		// Cari ID Kependudukan berdasarkan NIK
		if err := s.db.Where("nik = ?", fmt.Sprintf("%d", nik)).First(&kependudukan).Error; err != nil {
			if err != gorm.ErrRecordNotFound {
				return err
			}
			log.Printf("Kependudukan with NIK %d not found, fallback to basic child data\n", nik)
		} else {
			kID := kependudukan.IDKependudukan
			kependudukanID = &kID
		}
		...
		if detailAnak.IbuNik > 0 {
			var kepIbu models.Kependudukan
			if err := s.db.Where("nik = ?", fmt.Sprintf("%d", detailAnak.IbuNik)).First(&kepIbu).Error; err != nil {
				log.Printf("Kependudukan ibu with NIK %d not found, skipping child NIK %d\n", detailAnak.IbuNik, nik)
				continue
			}

			if err := s.db.Where("penduduk_id = ?", kepIbu.IDKependudukan).First(&ibu).Error; err != nil {
				log.Printf("Ibu with NIK %d not found, skipping child NIK %d\n", detailAnak.IbuNik, nik)
				continue
			}
		}
		...
		if kependudukanID != nil {
			err = s.db.Where("penduduk_id = ?", *kependudukanID).First(&existing).Error
		}
		...
		if err != nil {
			if err == gorm.ErrRecordNotFound {
				...
				// Skip for now if we can't create it properly
				log.Printf("Skipping Anak creation for %s due to model mismatch in seed logic", detailAnak.NamaAnak)
			} else {
				return err
			}
		}
