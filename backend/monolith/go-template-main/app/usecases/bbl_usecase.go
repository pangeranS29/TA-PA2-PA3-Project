...
type BblUsecase interface {
	...
	Verify(bblID uint, kaderID uint, periodeWaktu string) (*models.Bbl, error)
...
func (u *bblUsecase) Verify(bblID uint, kaderID uint, periodeWaktu string) (*models.Bbl, error) {
	if err := u.bblRepo.Verify(bblID, kaderID, periodeWaktu); err != nil {
		return nil, err
	}
